#!/usr/bin/env bash
# End-to-end проверка стенда.
#
# Пятый шаг — главный: тот же голосующий отправляет голос на ДВА разных
# инстанса, и после дренажа в результатах обязан оказаться ровно один голос.
# Это прямое доказательство того, ради чего дедуп живёт в Redis, а не в памяти
# процесса.
set -euo pipefail
cd "$(dirname "$0")/.."
...
# Ждём, пока консьюмер применит оба сообщения и снапшотер положит агрегат.
ballots=""
