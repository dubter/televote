#!/usr/bin/env bash
# Ждёт, пока оба инстанса начнут отвечать на /readyz.
#
# /readyz проверяет настоящие Postgres, Redis и Kafka, поэтому его 200 —
# это действительно «можно голосовать», а не «процесс запустился».
set -euo pipefail
cd "$(dirname "$0")/.."
