...
//
// Горячий путь читает их на каждый голос, поэтому чтение обязано быть
// неблокирующим и никогда не ходить в сеть. Обновление — фоновым рефрешером, а
// не ленивым TTL: истечение при 2M RPS дало бы thundering herd из тысяч
// одновременных промахов в Postgres, которого на горячем пути быть не должно.
package pollcfg
