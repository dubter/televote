...
//
// Это не поток и не очередь: снапшотер периодически ПЕРЕСЧИТЫВАЕТ ПОЛНОЕ
// СОСТОЯНИЕ — читает абсолютные значения и записывает абсолютные. Отсюда всё
// остальное: пропущенный цикл догоняется следующим, порядок циклов не важен,
// повторный цикл безвреден. Пиши он дельты, ретрай завышал бы результат.
package snapshot
