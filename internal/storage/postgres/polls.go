...
// SaltLen — длина соли опроса в байтах. Ровно размер блока ключа HMAC-SHA256,
...
type PollRow struct {
	...
	// Salt — соль вывода voter_id для ЭТОГО опроса. Генерирует сервер при
	// создании; клиент на неё влиять не может. Пер-опрос, а не глобальная:
	// общая соль делала бы voter_id сопоставимыми между опросами, то есть
	// восстанавливала бы историю голосований человека.
	Salt []byte
