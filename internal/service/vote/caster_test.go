...
func TestNewCaster_RejectsBrokenDependencies(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, err := vote.NewCaster(client, tc.ttl, tc.jitter, 250*time.Millisecond)
			assert.Error(t, err)
		})
	}

	c, err := vote.NewCaster(client, 30*time.Minute, 0.1, 250*time.Millisecond)
...
func TestCast_RejectsInvalidArgumentsBeforeRedis(t *testing.T) {
	...
	c, err := vote.NewCaster(struct{ rueidis.Client }{}, 30*time.Minute, 0.1, 250*time.Millisecond)
	require.NoError(t, err)
