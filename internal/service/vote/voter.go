...
func ParseVoterID(s string) (VoterID, error) {
	...
	if len(s) != hex.EncodedLen(len(v)) {
		return VoterID{}, fmt.Errorf("voterID: expected %d hex characters, got %d",
...
func DeriveVoterID(salt []byte, clientID string) (VoterID, error) {
	if len(salt) < minSaltLen {
		return VoterID{}, fmt.Errorf("%w: length %d, minimum %d", ErrBadSalt, len(salt), minSaltLen)
	}
	if len(clientID) > maxClientIDLen {
		return VoterID{}, fmt.Errorf("%w: length %d, maximum %d", ErrBadClientID, len(clientID), maxClientIDLen)
	}
