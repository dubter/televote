...
import (
	...
	"github.com/dubter/televote/internal/service/counting/mocks"
...
func newTestCounting(t *testing.T, cfg Config) (
	*Service, *mocks.MockApplier, *mocks.MockConfigLookup, *mocks.MockObserver,
) {
	t.Helper()
	...
	svc, err := New(applier, lookup, obs, slog.New(slog.NewTextHandler(io.Discard, nil)), cfg)
	require.NoError(t, err)
	return svc, applier, lookup, obs
}

func message(t *testing.T, cfg *pollcfg.HotConfig, clientID string, choices []uint8, at time.Time) domain.VoteMessage {
	t.Helper()

	v, err := domain.DeriveVoterID(cfg.Salt, clientID)
	require.NoError(t, err)
	return domain.VoteMessage{PollID: cfg.ID, VoterID: v.Hex(), Choices: choices, ProducedAt: at}
}
...
func TestCounting_AppliesVoteExactlyOnceWithPollShardCount(t *testing.T) {
	...
	c.Count(context.Background(), message(t, cfg, "viewer", []uint8{1}, opensAt.Add(time.Second)))
}
...
func TestCounting_DuplicateDeliveryDoesNotDoubleCount(t *testing.T) {
	...
	msg := message(t, cfg, "viewer", []uint8{2}, opensAt.Add(time.Second))
	for range 5 {
		_, err := c.Count(context.Background(), msg)
		require.NoError(t, err)
	}
}
...
func TestCounting_UsesProducedAtNotProcessingTime(t *testing.T) {
	...
	c.Count(context.Background(), message(t, cfg, "late", []uint8{0}, closesAt.Add(-time.Second)))
}
...
func TestCounting_RejectsVoteProducedOutsideWindow(t *testing.T) {
	...
	c.Count(context.Background(), message(t, cfg, "early", []uint8{0}, opensAt.Add(-time.Second)))
	c.Count(context.Background(), message(t, cfg, "late", []uint8{0}, closesAt.Add(time.Second)))
}

func TestCounting_RejectsUnknownPollAndBadVoter(t *testing.T) {
	t.Parallel()
	...
	_, err := c.Count(context.Background(), domain.VoteMessage{
	...
	assert.ErrorIs(t, err, domain.ErrInvalidVote)
}
...
func TestCounting_RetriesRetryableErrorUntilSuccess(t *testing.T) {
	...
	c.Count(context.Background(), message(t, cfg, "viewer", []uint8{0}, opensAt.Add(time.Second)))
}
...
func TestCounting_HoldsPartitionInsteadOfDroppingVote(t *testing.T) {
	...
	c.Count(short, message(t, cfg, "viewer", []uint8{0}, opensAt.Add(time.Second)))
}
...
func TestCounting_BreakerStopsCallingRedisAfterErrorRatio(t *testing.T) {
	...
	for i := range 40 {
		...
		c.Count(ctx, message(t, cfg, string(rune('a'+i%26)), []uint8{0}, opensAt.Add(time.Second)))
		cancel()
	}
