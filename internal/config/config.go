...
//
// Валидация происходит один раз при старте и падает целиком (fail fast):
// сервис, поднявшийся с конфигом, нарушающим инвариант дедупликации, не даст
// ни ошибки, ни записи в логе — он просто будет тихо принимать повторные голоса.
package config
