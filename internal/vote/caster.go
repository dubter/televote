...
//
// Джиттер обязателен: 30 млн ключей с одинаковым сроком истекут одновременно,
// и active expiry Redis добьёт кластер ровно на хвосте дренажа, где ещё
// считаются последние голоса.
//
// Смещение выводится из voterID, а не из случайного источника: SET NX не
// продлевает существующий ключ, поэтому повторная доставка того же сообщения
// обязана считать тот же TTL.
...
//
// Ошибка классификации стоит дорого в обе стороны: ретрай неретраябельной
// ошибки заклинит партицию навсегда, а отказ от ретрая транзиентной потеряет
// голос. Поэтому список постоянных ошибок явный, а всё неизвестное считается
// транзиентным — сетевые сбои приходят без сентинелов и составляют
// подавляющее большинство отказов.
