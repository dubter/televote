...
//
// Инвариант «дедуп-ключ и счётчик в одном слоте» проверяется здесь без Docker:
// verify-invariants.sh ломает keys.go и запускает `go test` БЕЗ тега
// integration, поэтому тест инварианта обязан быть юнитом. Расчёт слота
// сверяется с настоящим сервером в integration-тесте
// TestCast_NoCrossSlotError (CLUSTER KEYSLOT), так что независимая копия
// алгоритма не расходится с Redis молча.
